...
impl GenesisParams {
    ...
    /// SigmaChain testnet genesis. Embeds the single emission box
    /// (176,525,325 YOLO + singleton emission NFT + R4/R5 holding the
    /// blake2b256 hashes of the treasury and LP accumulation
    /// scripts) generated by
    /// `07-sigmachain-node/sigmachain-genesis-tool`.
    ///
    /// `header_id` stays `None` for now (the height-1 header doesn't
    /// exist until the first block is mined). `state_digest` is a
    /// zero placeholder; the node computes the real AVL+ digest from
    /// the embedded boxes at startup. A follow-up commit will pin the
    /// computed digest here once a node has been booted against the
    /// genesis state — same procedure Ergo uses for new testnets
...
mod tests {
    ...
    fn sigmachain_testnet_genesis_embeds_emission_box_json() {
        // Phase 4.3: genesis_boxes.json is now embedded; header_id and
        // state_digest stay placeholder pending a first-block boot.
        let g = GenesisParams::sigmachain_testnet();
        // state_digest stays the zero placeholder until a node boots
        // against the genesis state and the AVL+ digest is captured.
        ...
        assert!(boxes.starts_with('['));
        // Exactly one box at genesis: the emission box.
        assert_eq!(boxes.matches("\"value\"").count(), 1);
        // Embeds the emission NFT in `assets`.
        assert!(boxes.contains("\"assets\""));
        assert!(boxes.contains("\"tokenId\""));
        // Embeds the treasury (R4) and LP (R5) script hashes.
        assert!(boxes.contains("\"R4\""));
        assert!(boxes.contains("\"R5\""));
        // The genesis emission box value matches the .5-fixed total
        // supply (176,525,325 YOLO in nano units, see
        // 01-emission-tests/emission_model.py genesis_box_value()).
        assert!(boxes.contains("176525325000000000"));
    }
