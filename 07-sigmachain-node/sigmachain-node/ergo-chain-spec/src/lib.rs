...
impl GenesisParams {
    ...
    ///
    /// `state_digest` is the 33-byte AVL+ `ADDigest` (32-byte root +
    /// 1-byte tree-height marker) computed deterministically from the
    /// embedded JSON by inserting each `(box_id, serialize_ergo_box(b))`
    /// pair into a fresh `AvlTree`. The reproducibility test lives in
